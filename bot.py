...
def get_token_from_query(query):
...
def send_spin_request(user_id, token):
    ...
    while True:
        try:
            ...
            if response.status_code == 201:
                logger.info(f"Spin request untuk user ID {user_id} berhasil! ({response.status_code})")
            else:
                logger.warning("Spin habis, menunggu loop")
                break
            time.sleep(3)  # Tunggu 3 detik sebelum request berikutnya
        except Exception as e:
...
def process_account(query, index):
    logger.info(f"[bold cyan]=== Memproses Akun {index} ===[/bold cyan]")  # Markup akan di-render
    ...
    token = get_token_from_query(query)
    if token:
        logger.info(f"Akun {index}: Token diperoleh dari query.")
        send_spin_request(user_id, token)
